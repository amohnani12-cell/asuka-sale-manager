...
const setAllPct=(pct)=>{
  const n={};
  pageProd.forEach(p=>{n[p.id]={pct:String(pct),exact:''};});
  setInputs(prev=>({...prev,...n}));
};

const clearPage=()=>{
  const n={...inputs};
  pageProd.forEach(p=>delete n[p.id]);
  setInputs(n);
};
  ...
  for(const p of toSave){
    ...
    try{
      ...
      if(d.variant){
        ...
        done++;setStatus('Saved '+done+'/'+toSave.length+'...');
      }
    }catch(e){}
  }
  clearPage();
...
const removeSale=async(p)=>{
  try{
    const r=await fetch('/api/update-variant?variantId='+p.variantId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({variant:{id:p.variantId,price:p.compareAt.toString(),compare_at_price:''}})});
    const d=await r.json();
    if(d.variant){
      setProducts(prev=>prev.map(x=>x.id===p.id?{...x,price:p.compareAt,compareAt:null}:x));
      setStatus('Removed sale: '+p.title);
    }
  }catch(e){}
};

const G='#006B4F',GOLD='#C9A84C',R='#B91C1C',D='#111827',M='#4B5563',B='#E5E7EB';
const discountedOnPage=pageProd.filter(p=>getSale(p)!==null).length;
...
<div style={{background:'white',borderBottom:'1px solid '+B,padding:'8px 20px',display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',flexShrink:0}}>
  ...
  <div style={{marginLeft:'auto',display:'flex',gap:6,alignItems:'center'}}>
    <span style={{fontSize:11,color:M}}>Set all:</span>
    {[10,15,20,25,30,40,50].map(v=><button key={v} onClick={()=>setAllPct(v)} style={{padding:'3px 9px',background:'#F9FAFB',border:'1px solid '+B,borderRadius:20,fontSize:11,fontWeight:600,cursor:'pointer',color:M}}>{v}%</button>)}
    <button onClick={clearPage} style={{padding:'3px 10px',background:'#FEF2F2',border:'1px solid #FECACA',borderRadius:20,fontSize:11,fontWeight:600,cursor:'pointer',color:R,marginLeft:4}}>Clear</button>
  </div>
</div>
    ...
    {pageProd.map(p=>{
      ...
      return(<div key={p.id} style={{background:'white',borderRadius:10,border:'1px solid '+(sale?'#A7D7C5':B),boxShadow:'0 2px 8px rgba(0,0,0,.06)',overflow:'hidden',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{padding:'6px 10px 8px',borderTop:'1px solid #F3F4F6',display:'flex',flexDirection:'column',gap:5}}>
          ...
          {p.compareAt&&<button onClick={()=>removeSale(p)} style={{width:'100%',padding:'4px',background:'#FEF2F2',border:'1px solid #FECACA',borderRadius:6,fontSize:10,fontWeight:600,cursor:'pointer',color:R,fontFamily:'inherit'}}>✕ Remove Sale</button>}
        </div>
      </div>);
    })}
  </div>
...
<div style={{background:'white',borderTop:'1px solid '+B,padding:'10px 20px',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
  ...
  <button onClick={saveAll} disabled={saving||discountedOnPage===0} style={{background:G,color:'white',border:'none',padding:'10px 28px',borderRadius:10,fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:(saving||discountedOnPage===0)?.5:1}}>
    {saving?'Saving...':'✓ Save '+(discountedOnPage||'')+(discountedOnPage?' products':'All')+' & Next'}
  </button>
</div>
