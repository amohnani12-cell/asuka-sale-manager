...
const saveAll=async()=>{
  ...
  if(!t.length){setStatus('No discounts set — enter % or price first');return;}
...
<div style={{background:'#F9FAFB',borderBottom:'1px solid '+B,padding:'7px 20px',display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',flexShrink:0}}>
  ...
  <span style={{fontSize:11,color:M,fontWeight:600}}>Apply to {someSelected?'selected ('+selectedIds.length+')':'all on page'}:</span>
  ...
  <span style={{color:'#D1D5DB',fontSize:13}}>|</span>
  <button onClick={bulkRemove} disabled={saving} style={{padding:'3px 10px',background:'#FEF2F2',border:'1px solid #FECACA',borderRadius:6,fontSize:11,fontWeight:600,cursor:'pointer',color:R}}>✕ Remove Sale {someSelected?'('+selectedIds.filter(id=>{const p=products.find(x=>x.id==id);return p&&p.compareAt;}).length+')':'(all on page)'}</button>
...
<div style={{flex:1,overflowY:'auto',padding:'12px 20px'}}>
  <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10}}>
    {pageProd.map(p=>{
      const inp=inputs[p.id]||{};const sale=getSale(p);const isSel=!!selected[p.id];
      return(<div key={p.id} style={{background:'white',borderRadius:10,border:'2px solid '+(isSel?D:sale?'#A7D7C5':B),boxShadow:'0 2px 8px rgba(0,0,0,.06)',overflow:'hidden',display:'flex',flexDirection:'column',position:'relative'}}>
